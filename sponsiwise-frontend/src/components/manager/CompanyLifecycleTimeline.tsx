import type { CompanyTimelineEntry } from "@/lib/types/manager";

interface CompanyLifecycleTimelineProps {
    timeline: CompanyTimelineEntry[];
}

const typeColors: Record<string, { dot: string; border: string }> = {
    COMPANY_CREATED: { dot: "bg-blue-500", border: "border-blue-200" },
    COMPANY_VERIFIED: { dot: "bg-green-500", border: "border-green-200" },
    COMPANY_REJECTED: { dot: "bg-red-500", border: "border-red-200" },
...
/**
 * Vertical timeline specifically for company lifecycle views.
 */
export default function CompanyLifecycleTimeline({
    timeline,
}: CompanyLifecycleTimelineProps) {
