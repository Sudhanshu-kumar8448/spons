...
export default async function ManagerNotificationsPage({
  ...
  try {
    ...
    error = normalizeError(err, "Unable to load notifications. Please try again later.");
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Your recent notifications and alerts.</p>
        </div>
        <Link
      ...
      {error ? (
        ...
        <EmptyState icon="🔔" heading="No notifications" description="You don't have any notifications yet." />
      ) : (
        <NotificationList initialNotifications={notifications} total={total} page={page} pageSize={PAGE_SIZE} basePath="/manager/notification" />
      )}
    </div>
  );
}
